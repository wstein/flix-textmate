 ...
 *
 *   npm run extract:lexicon -- --flix-source <path-to-flix-checkout>
 *
...
const flixSource = resolveCheckout({
  flag: '--flix-source',
  env: 'FLIX_SOURCE',
  ...FLIX_COMPILER,
});
 ...
 *
 * Reports the upstream repository and commit rather than a filesystem path: the manifest is
 * committed and published, and where the checkout happened to live is neither reproducible
 * nor anyone else's business.
 */
function describeSource() {
  const git = (args) =>
    execFileSync('git', ['-C', flixSource, ...args], { encoding: 'utf8' }).trim();

  let origin = 'unknown source';
  try {
    // Normalise git@host:owner/repo.git and https://host/owner/repo.git to owner/repo.
    const url = git(['remote', 'get-url', 'origin']);
    origin = url.replace(/^.*[:/]([^/]+\/[^/]+?)(?:\.git)?$/, '$1');
  } catch {
    /* Not a git checkout; fall through. */
  }

  try {
    return `${origin} @ ${git(['rev-parse', '--short', 'HEAD'])}`;
  } catch {
    return origin;
  }
}
